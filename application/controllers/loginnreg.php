...
class loginnreg extends CI_Controller {
	...
	public function register_validation(){
		...
		if(strlen($this->input->post('first_name'))< 3){
			$this->session->set_userdata("error_first","Must be 3 characters.");
		}  
		if(empty($this->input->post('last_name'))){
			$this->session->set_userdata("error_last","Required Field.");
		}
		if(strlen($this->input->post('last_name'))< 3){
			$this->session->set_userdata("error_last","Must be 3 characters."); 
		}
